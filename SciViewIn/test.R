...
##--- reformat existing studies
if(F){
  file1 = "/home/rstudio/data/SCS/scRNA_RNA_HS_DCMAdipocytes_10x.db"
  file2 = "/home/rstudio/data/SCS/scRNA_RNA_HS_IlleumChrohnsDisease_10X.db"
  file3 = "/home/rstudio/data/SCS/scRNA_RNA_HS_healthyCholangiocytes_10x.db"

  connGenes <- RSQLite::dbConnect(RSQLite::SQLite(),file1)
  RSQLite::dbListTables(connGenes)
  query <- paste0("SELECT * FROM RNA_HS_DCMAdipocytes_10x_study")
  g <- RSQLite::dbGetQuery(connGenes, query)
  g$ORGANISM <- "Homo sapiens"
  dbWriteTable(connGenes,'RNA_HS_DCMAdipocytes_10x_study',g,overwrite=T)
  dbDisconnect(connGenes)
  
  connGenes <- RSQLite::dbConnect(RSQLite::SQLite(),file2)
  RSQLite::dbListTables(connGenes)
  query <- paste0("SELECT * FROM RNA_HS_IlleumChrohnsDisease_10X_study")
  g1 <- RSQLite::dbGetQuery(connGenes, query)
  g1$assay <- "RNA"
  g1$ORGANISM
  dbWriteTable(connGenes,'RNA_HS_IlleumChrohnsDisease_10X_study',g1,overwrite=T)
  g2 <- RSQLite::dbGetQuery(connGenes, query)
  
  connGenes <- RSQLite::dbConnect(RSQLite::SQLite(),file3)
  RSQLite::dbListTables(connGenes)
  query <- paste0("SELECT * FROM RNA_HS_healthyCholangiocytes_10x_study")
  g <- RSQLite::dbGetQuery(connGenes, query)
  g$ORGANISM <- "Homo sapiens"
  dbWriteTable(connGenes,'RNA_HS_healthyCholangiocytes_10x_study',g,overwrite=T)
  dbDisconnect(connGenes)
